...
export async function ensureStripeCustomerBillingSchema(sql: Sql): Promise<void> {
  const run = (ddl: string) => sql.unsafe(ddl);

  await run(`ALTER TABLE portal.agents ADD COLUMN IF NOT EXISTS stripe_customer_id TEXT`);

  // Promote an order-level Stripe Customer to the account only when the
  // historical evidence is unambiguous. Active subscriptions win; agents with
  // conflicting active Customer IDs are deliberately left for manual review.
  await run(`
...
/** Additive identity expansion: person -> verified emails -> provider subjects. */
export async function ensureAgentIdentitySchema(sql: Sql): Promise<void> {
  const run = (ddl: string) => sql.unsafe(ddl);
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS portal.agent_email_addresses (
      ...
      CONSTRAINT agent_email_addresses_kind_check
        CHECK (kind IN ('login','business','personal','recovery','public'))
    )`);
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_email_addresses_lower
    ON portal.agent_email_addresses(lower(email))`);
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_email_addresses_primary
    ON portal.agent_email_addresses(agent_id) WHERE is_primary = TRUE`);
  await run(`CREATE INDEX IF NOT EXISTS idx_agent_email_addresses_agent
  ...
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_login_identity_provider_subject
    ON portal.agent_login_identities(provider, provider_subject)`);
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_login_identity_primary
    ON portal.agent_login_identities(agent_id)
    WHERE is_primary = TRUE AND disabled_at IS NULL`);
  await run(`CREATE INDEX IF NOT EXISTS idx_agent_login_identities_agent
    ON portal.agent_login_identities(agent_id)`);
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_agent_merge_history_target
    ON portal.agent_merge_history(target_agent_id)`);

  // `portal` is not exposed by this project, but RLS remains defense in depth
  // if Data API settings change later. The application database role bypasses
  // RLS; no anon/authenticated policies are intentionally granted.
  await run(`ALTER TABLE portal.agent_email_addresses ENABLE ROW LEVEL SECURITY`);
  await run(`ALTER TABLE portal.agent_login_identities ENABLE ROW LEVEL SECURITY`);
  await run(`ALTER TABLE portal.agent_merge_history ENABLE ROW LEVEL SECURITY`);

  // Every legacy Agent starts with one verified primary login address. Google
  // provider subjects are learned on the next successful sign-in.
  await run(`
...
export async function ensureSchema(sql: Sql) {
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS portal.licensed_companies (
      id TEXT PRIMARY KEY CHECK (id IN ('homix_realty','homix_living')),
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS portal.teams (
      id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
      name TEXT NOT NULL,
      company_id TEXT REFERENCES portal.licensed_companies(id) ON DELETE RESTRICT,
      leader_agent_id INTEGER,
      status TEXT NOT NULL DEFAULT 'active',
      notes TEXT
    )`);
  await run(`ALTER TABLE portal.teams ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active'`);
  await run(`ALTER TABLE portal.teams ADD COLUMN IF NOT EXISTS company_id TEXT`);
  await run(`ALTER TABLE portal.teams DROP CONSTRAINT IF EXISTS teams_status_check`);
  await run(`ALTER TABLE portal.teams ADD CONSTRAINT teams_status_check
  ...
  await ensureStripeCustomerBillingSchema(sql);
  ...
  // Only perform the legacy backfill when introducing the column. An order link
  // was written by the old handler after successful processing. NULL links are
  // ambiguous and must stay retryable; never enqueue/replay historical events.
  // The new webhook also recognizes order links written by the old deployment
  // after this backfill, covering the migration-to-deployment handover window.
  await run(`DO $$ BEGIN
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS portal.onboarding_invitations (
      ...
      lock_company BOOLEAN NOT NULL DEFAULT FALSE,
  ...
  await run(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'team_compensation_configs_team_id_fk'
          AND confdeltype = 'c'
      ) THEN
  ...
  await run(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'team_join_requests_team_id_fk'
          AND confdeltype = 'r'
      ) THEN
      ...
      END IF;
    END $$`);
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_team_join_requests_pending_agent
    ON portal.team_join_requests(agent_id) WHERE status = 'pending'`);
  await run(`CREATE INDEX IF NOT EXISTS idx_team_join_requests_team_status
    ON portal.team_join_requests(team_id, status)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_team_join_requests_agent_created
  ...
  await run(`ALTER TABLE portal.team_leader_applications
    ADD COLUMN IF NOT EXISTS licensed_company TEXT,
    ADD COLUMN IF NOT EXISTS company_id TEXT`);
  await run(`UPDATE portal.team_leader_applications AS application
  ...
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_team_leader_applications_open_agent
    ON portal.team_leader_applications(applicant_agent_id)
    WHERE status IN ('submitted','approved')`);
  await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_team_leader_applications_team
    ON portal.team_leader_applications(team_id) WHERE team_id IS NOT NULL`);
  await run(`CREATE INDEX IF NOT EXISTS idx_team_leader_applications_status_created
  ...
  await run(`
    UPDATE portal.teams AS team SET company_id = leader.licensed_company_id
    FROM portal.agents AS leader
    WHERE team.leader_agent_id = leader.id AND team.company_id IS NULL`);
  await run(`
    ...
    WHERE application.applicant_agent_id = applicant.id AND application.company_id IS NULL`);
  await run(`
    UPDATE portal.onboarding_invitations AS invitation
    SET company_id = team.company_id, lock_company = TRUE
    FROM portal.teams AS team
    WHERE invitation.team_id = team.id AND invitation.company_id IS NULL`);
  await run(`
    DO $$ BEGIN
      ...
      END IF;
    END $$`);
  await run(`CREATE INDEX IF NOT EXISTS idx_agents_licensed_company_id ON portal.agents(licensed_company_id)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_teams_company_id ON portal.teams(company_id)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_onboarding_invitations_company_id ON portal.onboarding_invitations(company_id)`);
  await run(`
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_onboarding_events_agent_created
    ON portal.onboarding_events(agent_id, created_at)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_onboarding_events_invitation_created
    ON portal.onboarding_events(invitation_id, created_at)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_onboarding_events_team_created
