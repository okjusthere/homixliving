            ...
            <Card>
              ...
              <div className="p-6">
                ...
                <div className="mt-6 space-y-3 text-[13px]">
                  <div className="flex justify-between gap-3" style={{ color: tone.ink }}>
                    <span>{t.grossCommissionRow}</span>
                    <span className="shrink-0 font-mono">${fmtMoney(Number(saleDeal.grossCommission || 0))}</span>
                  </div>
                  {saleDeal.referralAmount ? (
                    <div className="flex justify-between gap-3" style={{ color: tone.amber }}>
                      <span>{t.referral}</span>
                      <span className="shrink-0 font-mono">-${fmtMoney(Number(saleDeal.referralAmount))}</span>
                    </div>
                  ) : null}
                  {saleDeal.brokerageFee ? (
                    <div className="flex justify-between gap-3" style={{ color: tone.amber }}>
                      <span>{t.brokerageAdminFee}</span>
                      <span className="shrink-0 font-mono">-${fmtMoney(Number(saleDeal.brokerageFee))}</span>
                    </div>
                  ) : null}
