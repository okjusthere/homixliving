...
function cleanDecisionInput(body: Record<string, unknown>) {
  ...
  const teamCapCents =
    body.teamCapCents == null || body.teamCapCents === ""
      ? null
  ...
  if (!isTeamSplitPreset(defaultTeamSplitPct))
    throw new Error("INVALID_TEAM_SPLIT");
  if (!isTeamSourcedSplitPreset(teamLeadSplitPct))
    throw new Error("INVALID_LEAD_SPLIT");
  ...
  if (action === "approve") {
    try {
      ...
      const messages: Record<string, string> = {
        ...
        INVALID_LEAD_SPLIT:
          "Team-sourced Split must be 10%, 15%, 20%, 25%, or 30%.",
        INVALID_CAP: "Invalid Team Cap.",
      };
      return NextResponse.json(
        { error: messages[code] || "Invalid decision" },
        { status: 400 },
      );
    }
  }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const eligibility = teamLeaderApplicationEligibility({
        ...
        affiliationContractComplete: affiliationContractComplete(applicant),
    ...
    await logAudit(
        ...
        : "Declined Team Leader application",
      {
        ...
        decisionReason: result.application.decisionReason,
      },
    );
    await notify({
      ...
      title:
        action === "approve"
          ? "Team Leader application approved"
          : "Team Leader application update",
      body:
        action === "approve"
          ? "Your forming team is ready. Complete the Team Leader agreement to unlock recruiting."
          : result.application.decisionReason ||
            "Your Team Leader application was not approved.",
    ...
    const mapped: Record<string, [string, number]> = {
      ...
      APPLICANT_NOT_ELIGIBLE: [
        "The applicant must still be an active, fully signed Solo Pro agent with a supported licensed company.",
        409,
      ],
      LICENSED_COMPANY_CHANGED: [
        "The applicant's licensed company changed after submission. Decline this application and ask them to apply again.",
        409,
      ],
    };
    if (mapped[code]) {
